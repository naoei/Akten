...
class Akten {
    ...
    _CreateWriteHeader() {
        ...
        let endOfFile = new EndOfFileBlock().write(Buffer.concat([ text, num, metadata, levelMetadata, channels ]));

        fs.writeFileSync(this.path, Buffer.concat([ text, num, metadata, levelMetadata, channels, endOfFile ]));
    }
}
